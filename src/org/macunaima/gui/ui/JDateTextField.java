...
public class JDateTextField extends JTextField {
	...
	public static final int JDATETEXTFIELD_FIRST_DAY_MONTH = 0;
	public static final int JDATETEXTFIELD_LAST_DAY_MONTH = 1;
	...
	public void setDate(int option) {
		if (option == JDATETEXTFIELD_FIRST_DAY_MONTH) {
			setDate(getFirstDayMonth());
		} else if (option == JDATETEXTFIELD_LAST_DAY_MONTH) {
			setDate(getLastDayMonth());
		} else {
			setDate(new Date());
		}
	}

	private static Date getFirstDayMonth() {
		Calendar c = Calendar.getInstance(); // this takes current date
		c.set(Calendar.DAY_OF_MONTH, 1);
		return c.getTime();
	}

	private static Date getLastDayMonth() {
	...
	private void showDateDialog(DatePanel dateChooser, Point position) {
		Frame owner = (Frame) SwingUtilities.getWindowAncestor(JDateTextField.this);
	...
	private class DatePanel extends JPanel implements ChangeListener {
		...
		Color controlLineColor = new Color(143, 60, 185);
